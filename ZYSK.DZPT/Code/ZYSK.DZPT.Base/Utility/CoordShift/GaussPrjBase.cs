...
namespace ZYSK.DZPT.Base.Utility
{
    ...
    {        
        ...
        {           
            ...
            //ZoneWide = 6; //6度带宽 
            //ProjNo = (int)(longitude / ZoneWide);  //代号
            //longitude0 = ProjNo * ZoneWide + ZoneWide / 2; //中央子午线

            //ZoneWide = 3; //3度带宽 
            ProjNo = (int)((longitude+1.5) / ZoneWide);
        ...
        #region 3°带高斯左边反算，（Y坐标无带号（此处是X坐标））
        /// <summary>
        /// 高斯左边反算-2000，Y坐标无带号（此处是X坐标）
        /// </summary>
        /// <param name="projNo">带号</param>
        /// <param name="X">正东方向</param>
        /// <param name="Y">正北方向</param>
        /// <param name="longitude">经度，以度为单位</param>
        /// <param name="latitude">纬度，以度为单位</param>
        public void GaussPrjInvCalculate_2000(int projNo,double X, double Y, out double longitude, out double latitude)
        {
            int ProjNo;
            double longitude1, latitude1, longitude0, latitude0, X0, Y0, xval, yval;
            double e1, e2, ee, NN, T, C, M, D, R, u, fai, iPI;
            ...
            //3度带
            ProjNo = projNo; //带号
            longitude0 = ZoneWide * ProjNo; //中央子午线
            longitude0 = longitude0 * iPI; //中央经线
            X0 = 500000L;
            Y0 = 0;
            xval = X - X0;
            ...
            //longitude = this.DDToD(longitude);
            //latitude = this.DDToD(latitude);

        }
        #endregion

        #region 3°带高斯左边反算，（Y坐标包含带号（此处是X坐标））
        /// <summary>
        ///  高斯左边反算-2000，Y坐标包含带号（此处是X坐标）
        /// </summary>
        ...
        public void GaussPrjInvCalculate_2000_DH(double X, double Y, out double longitude, out double latitude)
        {
            int ProjNo;
            double longitude1, latitude1, longitude0, latitude0, X0, Y0, xval, yval;
            double e1, e2, ee, NN, T, C, M, D, R, u, fai, iPI;
            ...
            //longitude0 = (ProjNo - 1) * ZoneWide + ZoneWide / 2;
            //3度带
            ProjNo =Convert.ToInt32( X.ToString().Substring(0,2)); //带号
            longitude0 = ZoneWide * ProjNo; //中央子午线
            longitude0 = longitude0 * iPI; //中央经线
            X0 = ProjNo*1000000L+500000L;
            Y0 = 0;
        ...
        //度分秒转度
        private double DDToD(double dfm)
        { 
            double du = 0;
            double  fen = 0;
            double  miao = 0;
            du=(int)dfm;
            fen=(int)((dfm-du)*100);
            miao=((dfm-du)*100-fen)*100;
            return du+(double)fen/60+(double)miao/3600;
